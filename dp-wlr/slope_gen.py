...
# Creates a sorted Point-Weight Matrix for a specific k, with appended lower and upper bounds
# If k == 0, do complete graph
# If k < n, do k samplings with inclusion
# Inputs: dataset, k - number of samplings, n - sample size of dataset

def create_point_matrix(df, k, n, lower_bound=0, upper_bound=1):

    # Initializes with lower and upper bounds

    point_list = [lower_bound,upper_bound]
    weight_list = [0,0]
    ...
    if k == 0:
        ...
        for i in range(n):
            for j in range(i+1,n):
                if (df[0][j] != df[0][i]):
                    slope = (df[1][j] - df[1][i]) / (df[0][j] - df[0][i])
                    xmean = (df[0][j]+df[0][i])/2 
                    ymean = (df[1][j]+df[1][i])/2
                    point_est = slope * 0.25 + ymean - (slope * xmean)
                    point_list.append(point_est)
                    weight_list.append(df[2][i] * df[2][j])
    ...
    # k-Match Case    
        ...
        for i in range(k):
            ...
            for j in range(n // 2):
                ...
                i_2 = ordering[2 * j + 1]
                if (df[0][i_1] != df[0][i_2]):
                    slope = (df[1][i_1] - df[1][i_2]) / (df[0][i_1] - df[0][i_2])
                    xmean = (df[0][i_1]+df[0][i_2])/2 
                    ymean = (df[1][i_1]+df[1][i_2])/2
                    point_est = slope * 0.25 + ymean - (slope * xmean)
                    point_list.append(point_est)
                    weight_list.append(df[2][i_1] * df[2][i_2])
    ...
    out_df = np.array([point_list, weight_list])
    out_df = out_df[:, out_df[0, :].argsort()]    
